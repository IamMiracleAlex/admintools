...
def handle():
    ...
    def get_urls():
        ...
        if tbaq:
            ...
            return {"details":"No url to archive at this time"}
        

    def post_archive(url, archived_url):
        """
        Updates the url
        """
    ...
    def default_archive(url):
        ...
        if lang == "de":
            ...
            archived_url = internal_archive(url)

        else:
            archived_url = wayback_machine(url)
    ...
    def url_archiver(method, url):
        if method == 'internal_tool':
            ...
            archived_url = default_archive(url)
        
        if archived_url:
            post_archive(url, archived_url=archived_url)
    ...
    if data.get('urls'):
        ...
        for url in urls:
            ...
            url_archiver(method=method, url=url)
